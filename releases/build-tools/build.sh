...
artifactory_default_url="https://repo.phdata.io/TOKEN/cf-gold-templates/raw/versions/"
...
post_pr_comment() {
    ...
    if [ "$repo_type" = "CODECOMMIT" ]; then
        ...
        comment=$(cat newlinecomment)
        aws codecommit post-comment-for-pull-request --pull-request-id "$pull_request_id" --repository-name "$(basename $CODEBUILD_SRC_DIR)" --before-commit-id "$before_commit_id" --after-commit-id "$after_commit_id" --content "$comment"
        pr_title=`aws codecommit get-pull-request --pull-request-id $pull_request_id | jq -r '.[].title'`
        # repo_url=`aws codecommit get-repository --repository-name "$(basename $CODEBUILD_SRC_DIR)" | jq -r ".[].cloneUrlHttp"`
        echo "" >> newlinecomment
        echo "This PR can be reviewed at:" >> newlinecomment
...
configure_aws_environment() {
    ...
    if [ "$1" == "caller" ];then
        ...
        env_detail="Environment name: $env_name | Account-Id: $account_id | Region: $AWS_REGION"
...
# calls cfci_plan once if plan is requested for one environment, same method is called in loop if plan is requested for multiple environments
process_plan () {

if [ -z "$plan_all" ] ; then
    plan_all=false
fi
# plan_all var is set and TRUE in buildspec then run PLAN for all environments when PR is created
if [ "$plan_all" = false ] || [ "$env_count" -lt 2 ] ; then
    ...
    do  
        ...
        cfci_plan
        # echo "$env_separator $nl_sep  $env_detail $nl_sep $nl_sep  $plan_comment $nl_sep $nl_sep " >> plan_comments
...
if [ "$stage" = "build" ]; then
    ...
    python stack_parser.py $CODEBUILD_SRC_DIR/$deployment_decriptor | tee parse_log
                ...
                if [ -z "$CODEBUILD_WEBHOOK_EVENT" ] ; then
                    if [ "$repo_type" = "bitbucket" ]; then
                        ...
                        if [ "$pr_status" == "MERGED" ] || [ "$pr_status" == "Closed" ]; then
                            #cfci_deploy
                            echo "ERROR: BUILD RETRY was executed on a PR which is already MERGED" >> output
                            exit 1
